...
def compute_data_dict(compounds, data):
    ...
    for (name, composition) in compounds.items():
        new_data[name] = {"category": "Блюдо", "price": {"item": 0, "kg": 0, "100g": 0}, "weight": 0,
                          "nutrients_in_100g": {}, "nutrients_total": {}}
        for item in composition:
            new_data[name]['weight'] += composition[item]
            if item in data:
                new_data[name]['price']['item'] += data[item]['price']['100g'] / 100 * composition[item]
                for (nutrient, amount) in data[item]['nutrients_in_100g'].items():
                    ...
                    new_data[name]['nutrients_total'][nutrient] += amount / 100 * composition[item]
        weight = new_data[name]['weight']
        price = new_data[name]['price']
        for (nutrient, amount) in new_data[name]['nutrients_total'].items():
            new_data[name]['nutrients_in_100g'][nutrient] = round(amount / weight * 100, 2)
