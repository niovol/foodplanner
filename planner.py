...
def plot_ration_nutrients(nutrient_ranges, ration_nutrients, title=''):
    ...
    for nutrient_idx, nutrient in enumerate(nutrient_ranges):
        normalized_nutrient_values = [value / max(nutrient_ranges[nutrient]) for value in nutrient_ranges[nutrient]]
        normalized_current_value = min(ration_nutrients.get(nutrient, 0) / max(nutrient_ranges[nutrient]), 1)
        range_idx = np.searchsorted(nutrient_ranges[nutrient], ration_nutrients.get(nutrient, 0))
        bar_color = bar_colors[range_idx]
        plt.barh(num_bars - 1 - ind[nutrient_idx], normalized_current_value, height=bar_height, color=bar_color, alpha=0.8, label=nutrient)

        for i, value in enumerate(normalized_nutrient_values):
            plt.plot([value, value], [num_bars - 1 - ind[nutrient_idx] - 0.5 * bar_height, num_bars - 1 - ind[nutrient_idx] + 0.5 * bar_height],
                     color=colors[i], linestyle='--', linewidth=1.5)
            plt.text(value, num_bars - 1 - ind[nutrient_idx] + 0.55 * bar_height, f'{nutrient_ranges[nutrient][i]:.1f}', fontsize=9, color=colors[i])

        plt.text(0.01, -0.1 + num_bars - 1 - ind[nutrient_idx], f'{ration_nutrients.get(nutrient, 0):.1f}',
                 fontsize=10, color='black', bbox=dict(facecolor='white', edgecolor='none', pad=1))
...
class DishWeightEntry(tk.Frame):
    def __init__(self, autocomplete_list, remove_callback, disable_autocomplete=False, *args, **kwargs):
        super().__init__(*args, **kwargs)

        self.dish_entry = AutocompleteEntry(autocomplete_list, disable_autocomplete=disable_autocomplete, master=self)
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.save_physiological_params_button = tk.Button(self.left_frame, text="Save Physiological Params", command=self.save_physiological_params)
        self.save_physiological_params_button.pack()

        self.load_physiological_params_button = tk.Button(self.left_frame, text="Load Physiological Params", command=self.load_physiological_params)
        ...
        self.gender_dropdown = ttk.Combobox(self.left_frame, textvariable=self.gender_var, values=["male", "female"], state="readonly")
        ...
        self.activity_multiplier_entry = ttk.Entry(self.left_frame, textvariable=self.activity_multiplier_var)
        ...
        self.breastfeeding_check = ttk.Checkbutton(self.left_frame, variable=self.breastfeeding_var)
        self.breastfeeding_check.pack()
        
        self.save_button = tk.Button(self.left_frame, text="Save Ration", command=self.save_ration)
        self.save_button.pack()

        self.load_button = tk.Button(self.left_frame, text="Load Ration", command=self.load_ration)
        self.load_button.pack()

        self.export_button = tk.Button(self.left_frame, text="Export Plot", command=self.export_plot)
        self.export_button.pack()

        self.add_dish_button = tk.Button(self.left_frame, text="Add Dish", command=self.add_dish_field)
        self.add_dish_button.pack()

        self.plot_button = tk.Button(self.left_frame, text="Plot", command=self.plot_ration_nutrients)
    ...
    def add_dish_field(self):
        dish_weight_entry = DishWeightEntry(list(self.data.keys()), remove_callback=self.remove_dish_field, master=self.left_frame)
    ...
    def load_ration(self):
        ...
        for dish, weight in ration_dict['Рацион'].items():
            dish_weight_entry = DishWeightEntry(list(self.data.keys()), remove_callback=self.remove_dish_field,
                                                disable_autocomplete=True, master=self.left_frame)
    ...
    def plot_ration_nutrients(self):
        ...
        nutrient_ranges = calculate_nutrient_ranges(gender, height, weight, age, activity_multiplier, breastfeeding)
        
        ration_dict = self.get_current_ration_dict()
        nutrients_total = compute_data_dict(ration_dict, self.data)['Рацион']['nutrients_total']
